...
`AdminService`는 controller가 호출하는 use case 계약입니다. View와 Controller는 endpoint 문자열, generated DTO, raw server payload, storage SDK를 직접 알면 안 됩니다.
...
│   ├── service/          # AdminService, gateway, mapper, upload/link services
...
- Controller: `src/api/controller`에서 TanStack Query mutation/query 조합, form orchestration, view model을 다룹니다.
- Domain/API: `src/api/model`과 `src/api/service`에서 domain contract, gateway, mapper, upload, QR, external link 정책을 다룹니다.
- QA: `src/test`와 colocated test에서 mapper/gateway/controller/component/e2e 회귀를 보호합니다.
...
- `taglow-api-boundary`: AdminService, Gateway, Mapper, OpenAPI 경계
...
- `api/controller/**`는 generated type, raw DTO, fetch client, S3 SDK를 import하지 않습니다.
...
- `api/service/gateway/**`는 React component, controller state, QR/player URL builder를 import하지 않습니다.
...
- Controller: role check, mutation invalidation, upload/API failure 분리, copy/download/open fallback을 검증합니다.

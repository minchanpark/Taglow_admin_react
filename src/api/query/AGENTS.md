...
`src/api/query`는 React Query hook과 page-facing view model을 둡니다. View event를 API Controller use case로 연결하고, TanStack Query 상태를 화면 친화적으로 정리합니다.

## 책임

- `useAuthQuery`, `useVoteListQuery`, `useVoteDetailQuery`, `useQuestionEditorQuery`를 관리합니다.
...
- browser side effect를 view에서 직접 하도록 넘기지 말고 runtime service/helper callback으로 감쌉니다.
